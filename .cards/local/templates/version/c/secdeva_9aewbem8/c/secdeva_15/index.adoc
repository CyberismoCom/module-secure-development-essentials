...
Internal controls should be in an acceptable state. See {{#report}}"name": "xref", "identifier": "secdeva_status-and-next-steps"{{/report}}.
...
Check that Security issues affecting this version have been tracked to closure or deferred to a future release with a justified reason. See {{#report}}"name": "xref", "identifier": "secdeva_status-and-next-steps"{{/report}}.
...
Check the status of automated policy checks in the project. See {{#report}}"name": "xref", "identifier": "secdeva_status-and-next-steps"{{/report}}.
