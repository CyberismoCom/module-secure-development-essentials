...
Internal controls should be in an acceptable state. See {{#report}}"name": "secdeva/reports/xref", "identifier": "secdeva_status-and-next-steps"{{/report}}.
...
Check that Security issues affecting this release have been tracked to closure or deferred to a future release with a justified reason. See {{#report}}"name": "secdeva/reports/xref", "identifier": "secdeva_status-and-next-steps"{{/report}}.
...
Check the status of automated policy checks in the project. See {{#report}}"name": "secdeva/reports/xref", "identifier": "secdeva_status-and-next-steps"{{/report}}.
...
Product documentation and technical documentation are reviewed by their own controls: see {{#report}}"name": "secdeva/reports/xref", "identifier": "secdeva_control_product-documentation-review"{{/report}} for the product documentation, and, where the EU Cyber Resilience Act applies, the technical documentation review provided by the EU CRA module. This check does not repeat those reviews. It confirms that their outcome still holds for what is being released now.

Check that

* The product documentation for users, including the security-related instructions, is up to date for this release
* The internal technical documentation is up to date for this release, if technical documentation is required for this product
* The documentation reviews cover the changes made in this release, and the reviewed versions of the documents are the ones that this release delivers
...
The conformity assessment procedure itself is not part of this review. It is managed as its own task, usually on the topmost project level, and the EU CRA module provides it. This check only confirms that it has been carried out and that its evidence exists for this release.

Mark this check as not applicable if the product is not placed on the EU market.

Check that

* The applicable EU conformity assessment procedure has been carried out
* The conformity assessment has been revisited if this release changes the design or the characteristics of the product in a way that affects conformity
* The evidence of the assessment, including the EU declaration of conformity, has been archived with the other release artifacts
